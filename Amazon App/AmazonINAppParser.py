...
def fetch_product_details(url,total_retries=3):
    ...
    while retry_count < total_retries:
        try:
            return requests.post('https://api.syphoon.com',json=payload).text
...
def main(url):
    ...
    makedirs(req_asin,exist_ok=True)

    
    try:

        with open(f'{req_asin}/{req_asin}.html','r',encoding='utf-8') as f:
            html = f.read()
    except:
        html = fetch_product_details(url+'?th=1&psc=1')
        
        with open(f'{req_asin}/{req_asin}.html','w',encoding='utf-8') as f:
            f.write(html)

    soup = BeautifulSoup(html, 'lxml')

    variation_info = get_variations(soup)
    with open(f'{req_asin}/varints.json','w') as f:
        json.dump(variation_info,f,indent=4)


    response_data = parse_amazon_page(soup,url=url,variation_info=variation_info)
    if response_data:
        reviews = 0
        review_url ='https://www.amazon.in/product-reviews/'+req_asin
        review_html = fetch_product_details(review_url)
        review_soup = BeautifulSoup(review_html, 'lxml')
        reviews = get_reviews(review_soup)
        response_data['Reviews'] = reviews

        if soup.select('span[data-action="show-all-offers-display"] a'):
            req = fetch_product_details(f'https://www.amazon.in/gp/product/ajax/ref=dp_aod_NEW_mbc?asin={req_asin}&m=&qid=1723543701&smid=&sourcecustomerorglistid=&sourcecustomerorglistitemid=&sr=8-4&pc=dp&experienceId=aodAjaxMain')
            response_data['Additional sellers'] = get_other_sellers(BeautifulSoup(req, 'lxml'))
    ...
    if variation_info:
        ...
        for asin in variation_info['asin_mapping']:
            try:
                url = 'https://www.amazon.in/dp/'+asin+'?th=1&psc=1'

                try:
                    with open(f'{req_asin}/{asin}.html','r',encoding='utf-8') as f:
                        html = f.read()
                except:
                    html = fetch_product_details(url)

                    with open(f'{req_asin}/{asin}.html','w',encoding='utf-8') as f:
                        f.write(html)

                soup = BeautifulSoup(html, 'lxml')
                response_data = parse_amazon_page(soup,url,asin,variation_info)
                response_data['Reviews'] = reviews
                if soup.select('span[data-action="show-all-offers-display"] a'):
                    req = fetch_product_details(f'https://www.amazon.in/gp/product/ajax/ref=dp_aod_NEW_mbc?asin={asin}&m=&qid=1723543701&smid=&sourcecustomerorglistid=&sourcecustomerorglistitemid=&sr=8-4&pc=dp&experienceId=aodAjaxMain')
                    response_data['Additional sellers'] = get_other_sellers(BeautifulSoup(req, 'lxml'))
        ...
        data = response_data
    with open(f'{req_asin}/{req_asin}.json','w') as f:
...
if __name__=='__main__':
    #  []
    for asin in ['B0C1NDB4B9','B0BPGSJM53','B0BSRSLJ77','B08D3PQ8Z6','B08R51FFSX','B08VGSPZFY','B09VDDMGX7']:
        print(main(f'https://www.amazon.in/gp/dp/{asin}'))
