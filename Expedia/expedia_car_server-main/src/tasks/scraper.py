...
SYPHOON_KEY = getenv("SYPHOON_KEY")

m = MongoClient()
q = rq.Queue(connection=Redis())

result_collection = m["expedia_car"]["result"]
...
def process_search_page(search_url, origin_url, page_num, max_retries = 3):
    netloc = urllib.parse.urlparse(search_url).netloc
    search_data = fetch_car_search_response(search_url=search_url, max_retries=max_retries)
    if search_data is None:
        return None
    
    if search_data['data'].get('carSearchOrRecommendations') and search_data['data']['carSearchOrRecommendations'].get('carSearchResults'):
        car_list = search_data['data']['carSearchOrRecommendations']['carSearchResults']
        for result in car_list['listings']:
            ...
            q.enqueue(fetch_car_detail_response, (detail_page_url, origin_url, page_num, max_retries))
    


def fetch_car_search_response(search_url, max_retries=3):
    netloc = urllib.parse.urlparse(search_url).netloc
...
def fetch_car_detail_response(detail_page_url, origin_url, page_num, max_retries=3):
    ...
    while max_retries >= 0:
        try:
            response = requests.post("https://api.syphoon.com", headers=headers, json=payload)
            response.raise_for_status()
            result_collection.insert_one({"result": response.json(), "origin_url": origin_url, "page": page_num})
...
def expedia_internal_search_crawler(search_url, max_results = 0):

    search_response = fetch_car_search_response(search_url)
    netloc = urllib.parse.urlparse(search_url).netloc
    if search_response:
        if search_response['data'].get('carSearchOrRecommendations') and search_response['data']['carSearchOrRecommendations'].get('carSearchResults'):
            ...
            for result in car_list['listings']:
                if result['__typename'] != "CarOfferCard":
                    continue
                detail_page_url = f'https://{netloc}{result["infositeURL"]["relativePath"]}'
                q.enqueue(fetch_car_detail_response, (detail_page_url, search_url, 1))

            limit = no_of_results if (max_results == 0 or max_results > no_of_results) else max_results
            total_pages = (limit//page_size) + 1

            if total_pages > 1:
                page_urls = [f"{search_url}&selPageIndex={i * page_size}" for i in range(1,total_pages)]
                for page,url in enumerate(page_urls,start=2):
                    q.enqueue(process_search_page, (url, search_url, page))
