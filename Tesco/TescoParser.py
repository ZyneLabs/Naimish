def product_parser(html):
    ...
    product_data["Photo URL"] = None
    if  product_json["defaultImageUrl"]:
        product_data["Photo URL"] = product_json["defaultImageUrl"].split("?")[0]

    product_data["Price"] = None
    product_data["Price_text"] = None

    try:
        if product_json['price'].get('actual'):
    ...
    if categories :=soup.select('nav[aria-label="breadcrumb"] li a'):
        ...
        product_data["Category URL"] = categories[
            -1
        ].get("href")

    product_data["Rich content presence"] = "Yes"
    product_data["Video presence"] = "No"
    product_data["Gallery pictures"] = 0
    product_data["Gallery images URLs"]  = None
