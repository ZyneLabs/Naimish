    ...
    async with Actor:
        ...
        while default_queue:
            ...
            for request in default_queue[:10]:
                ...
                request_tasks.append(process_request(request,Actor))
            ...
            for request,response in results:
                if response:
                    if response.status_code == 200:
                        ...
                        for key in ['search_alias','images','videos','addition_videos','categories','aplus_content','best_seller_rank','brand_store','frequently_bought_together','also_bought','top_reviews']:
                            if response_data.get(key,'') or response_data.get(key,'') == []:
