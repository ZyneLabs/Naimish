...
from common.utils import send_req_syphoon,PROXY_VENDOR
import json
import webcolors
import logging
...
logger = logging.getLogger('AutomalluaeCrawler')
logger.setLevel(logging.ERROR)
log_file_name = "AutomalluaeCrawler.log"
    ...
    while True:
        try:
            ...
            products = [
                ...
                'name':f'{item["make"]} {item["model"]} {item["modelGrade"]}',
                'make':item['make'],
                'model':item['model'],
                'odometer':item['odometer'],
                'year' : item['modelYear'],
                'color':get_color_name('#'+item['exteriorColours']) if item['exteriorColours'] else None,
                'location': None,
                'engine_size':item['engineCapacity'],
                'trim' : None,
                'price':item['price'],
                'image':item['image'],
                'post_data': None,
            ...
            if len(products) > 0:
                json.dump(products, open('Automalluae.json', 'a', encoding='utf-8'), indent=4)
            
        except Exception as ex:
                logger.error(f'{start_index} - {str(ex)} - {str(traceback.format_exc())}')
